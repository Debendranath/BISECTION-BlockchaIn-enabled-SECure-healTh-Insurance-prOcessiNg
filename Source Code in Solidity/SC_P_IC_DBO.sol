...
contract SC_P_IC_DBO
{
	...
	// Declaration and Definition of various Data Structure Constructs.
	...
	// Insurance company locks security deposit in the smart contract.
	...
	// Insurance company withdraws security deposit from the smart contract.
	...
	// User is interested to buy a policy, and hence initiates the following transaction.
     ...
     *  of the terms&condition file(intended policy) within some fixed time window. Othwerwise the buyerAddr can reclaim
     *  the locked money..
     **/
	...
	// If the insurer doesn't respond within the timeout period, users can withdraw their money. 
    ...
    function claimMoney(address System_Users_Info_address,address SC_P_HA_1_address,address SC_P_DBO_address,uint _policyID, uint _claimedAmount,uint _estimatedBillId, bytes32 _comm_k,address _dboAddr,uint _appID) public
    {
	  ...
	  // Conditions for making successful claim.	
	  ...
	  // DBO puts hash of Encrypted Patient's File in the BC before sending to the IC.	
	  ...
	  // IC locks the claimed amount first.
	  ...
	  \\ If IC doesn't lock the money equal to claimed amount with in timeout period, users should get back their policy price.
	  ...
	  // If policyholder doesn't reveal decryption key within timeout, Insurer can withdrawn lockeed amount.
	  ...
	  \\ Policyholder reveals the secret key.
	  ...
	  // Insurer approves the policyholder's claim.
	  ...
	  // If the Insurer doesn't aprrove within the timeout, Policyholder is given special provision to unlock the IC's locked money.
